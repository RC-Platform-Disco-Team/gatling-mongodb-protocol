...
case class MongoFindCommand(commandName: Expression[String], collection: Expression[String], query: Expression[String], limit: Int = 50,  sort: Option[Expression[String]] = None, hint: Option[Expression[String]] = None, checks: List[MongoCheck] = Nil) extends MongoCommand(checks)
