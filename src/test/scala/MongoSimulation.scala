...
class MongoSimulation extends Simulation {
  ...
  val scn = scenario("Mongo scenario")
    .exec(mongo("count before").collection("messages").count().skip(5).limit(7).hint("{\"_id\": -1}").check(count.lessThan(1).saveAs("messages_count")))
...
//    .exec(mongo("insert empty").collection("messages").insert("{\"_acc\": \"1\"}"))
